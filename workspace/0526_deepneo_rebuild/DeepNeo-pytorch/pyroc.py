...
def random_mixture_model(pos_mu=.6, pos_sigma=.1, neg_mu=.4, neg_sigma=.1, size=200):
    ...
    neg = [(0, random.gauss(neg_mu, neg_sigma),) for x in range(size // 2)]
    return pos + neg
...
class ROCData(object):
    ...
    def _trapezoidal_rule(self, curve_pts):
        ...
        cum_area = 0.0
        for ix, x in enumerate(curve_pts[:-1]):
            cur_pt = x
    ...
    def confusion_matrix(self, threshold, do_print=False):
        ...
        if do_print:
            print("\t Actual class")
            print("\t+(1)\t-(0)")
            print("+(1)\t{}\t{}\tPredicted".format(tp, fp))
            print("-(0)\t{}\t{}\tclass".format(fn, tn))
...
if __name__ == '__main__':
    ...
    print("ROC AUC: {}".format(str(roc.auc(roc_n))))
    print('Standard Error:  {}'.format(str(roc.calculateStandardError(roc_n))))
