...
def encode_data(data_provider):
    ...
    for hla, peptide, affinity, hla_seq in data_provider:
        encoding_matrix = np.zeros((9, 369))
        for i, peptide_aa in enumerate(peptide):
            for j, hla_aa in enumerate(hla_seq):
                if peptide_aa == "*" or hla_aa == "*":
                    encoding_matrix[i, j] = 0
                else:
                    encoding_matrix[i, j] = impact_matrix[aa_to_index[peptide_aa], aa_to_index[hla_aa]]
        encoded_matrices.append(encoding_matrix)
        affinity_values.append(affinity)

    encoded_matrices = np.array(encoded_matrices)
    affinity_values = np.array(affinity_values)

    return encoded_matrices, affinity_values
