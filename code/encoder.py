...
class plm_plm_sp2(Dataset):
    def __init__(self, data_provider, hla_emb_path_s, hla_emb_path_p, epi_emb_path_s, epi_emb_path_p):
        ...
        # Load the embeddings into memory at once (instead of reading from file each time)
        self.hla_embeddings_s = h5py.File(self.hdf5_path_s1, 'r', libver='latest')
        self.hla_embeddings_p = h5py.File(self.hdf5_path_p1, 'r', libver='latest')
        self.epi_embeddings_s = h5py.File(self.hdf5_path_s2, 'r', libver='latest')
        self.epi_embeddings_p = h5py.File(self.hdf5_path_p2, 'r', libver='latest')
    ...
    def __getitem__(self, idx):
        ...
        # Load HLA embeddings (s and p)
        hla_len = len(hla_seq)
        
        # HLA s embedding
        embedding_s = np.squeeze(self.hla_embeddings_s[hla_name][()])
        embedding_p = np.squeeze(self.hla_embeddings_p[hla_name][()])
        hla_embedding_s = torch.tensor(embedding_s, dtype=torch.float32)
        hla_embedding_p = torch.tensor(embedding_p, dtype=torch.float32)
        s_emb_dim = hla_embedding_s.shape[-1]
        p_emb_dim = hla_embedding_p.shape[-1]
        if hla_len < 269:
            hla_embedding_s = torch.cat([hla_embedding_s, torch.zeros(269 - hla_len, s_emb_dim)], dim=0)
            hla_embedding_p = torch.cat([hla_embedding_p, torch.zeros(269 - hla_len, p_emb_dim)], dim=0)
        else:
            hla_embedding_s = hla_embedding_s[:269]
            hla_embedding_p = hla_embedding_p[:269]

        # Load Epi embeddings (s and p)
        epi_embedding_s = np.squeeze(self.epi_embeddings_s[epi_seq][()])
        epi_embedding_s = torch.tensor(epi_embedding_s, dtype=torch.float32)
        epi_embedding_p = np.squeeze(self.epi_embeddings_p[epi_seq][()])
        epi_embedding_p = torch.tensor(epi_embedding_p, dtype=torch.float32)
    ...
    def __del__(self):
        # Close the hdf5 files when the object is deleted
        self.hla_embeddings_s.close()
        self.hla_embeddings_p.close()
        self.epi_embeddings_s.close()
        self.epi_embeddings_p.close()
